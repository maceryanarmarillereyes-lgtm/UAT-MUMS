...
- **2026-04-22 (Sheet-wide CASE duplicate detection coverage fix):**
  - **Edit — `public/js/services-grid.js`:** Added full-grid duplicate visual pass (`refreshDuplicateIndicators`) so all rendered CASE cells with duplicate values are marked immediately after render/load, not just newly edited inputs.
  - **Edit — `public/js/services-grid.js`:** Added debounced duplicate indicator repaint during CASE edits, centralized style apply/clear helpers, and detector-level `duplicateRowCount()` used by unload guard so duplicate safety checks reflect whole-sheet index state.
  - **Behavior contract update:** Duplicate enforcement now evaluates the entire loaded sheet dataset and maintains persistent warnings across all duplicate CASE values in current sheet state.
