...
async function boot(){
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  QB CASE DETAIL MODAL — Global controller
  //  Called by my_quickbase.js when a row number button is clicked.
  //  Reads column map from the row snapshot to auto-detect field labels.
  // ══════════════════════════════════════════════════════════════════════════
  (function initQbCaseDetailModal() {
    var _allSnaps = [];
    var _curIdx   = 0;

    // ── Field label normalizer ──────────────────────────────────────────
    function _fieldVal(snap, labelKeywords) {
      ...
      var colId = Object.keys(snap.columnMap).find(function(id) {
        ...
        return labelKeywords.some(function(kw) { return lbl.includes(kw); });
      });
      if (!colId) return '';
    ...
    // ── Duration label (mirrors my_quickbase.js logic) ──────────────────
    function _durationLabel(val) {
      var numeric = Number(val);
      if (!isFinite(numeric) || numeric < 0) return String(val == null ? '' : val);
      var days = Math.round(numeric / (1000 * 60 * 60 * 24));
    ...
    function _statusBadge(s) {
      ...
      return '<span class="qb-status-badge ' + cls + '"><span class="qb-status-dot"></span>' + UI.esc(s || '—') + '</span>';
    }

    // ── Populate modal DOM from snapshot ───────────────────────────────
    function _populate(snap) {
      if (!snap) return;

      var caseId    = snap.recordId || '—';
      var desc      = _fieldVal(snap, ['short description', 'description', 'concern', 'subject', 'title']) || '—';
      var assigned  = _fieldVal(snap, ['assigned to', 'assigned', 'agent']) || '—';
      var contact   = _fieldVal(snap, ['contact', 'full name', 'customer name']) || '—';
      var endUser   = _fieldVal(snap, ['end user', 'client', 'account', 'customer']) || '—';
      var caseType  = _fieldVal(snap, ['type', 'category']) || '—';
      var status    = _fieldVal(snap, ['case status', 'status']) || '—';
      var rawAge    = _fieldVal(snap, ['age']);
      var rawLast   = _fieldVal(snap, ['last update days', 'last update', 'update days']);
      var latest    = _fieldVal(snap, ['latest update', 'last comment', 'latest', 'update on the case']) || '—';

      var age  = rawAge  ? _durationLabel(rawAge)  : '—';
      var last = rawLast ? _durationLabel(rawLast) : '—';
      ...
      set('qbcdDesc', desc);
      setHTML('qbcdStatusBadge', _statusBadge(status));
      set('qbcdMeta', 'Row ' + snap.rowNum + ' · ' + endUser);
      set('qbcdKpiAge', age);
      set('qbcdKpiLast', last);
      set('qbcdKpiType', caseType);
      set('qbcdKpiEndUser', endUser);
      set('qbcdAssigned', assigned);
      set('qbcdContact', contact);
      set('qbcdEndUser', endUser);
      set('qbcdCaseId2', caseId);
      set('qbcdType2', caseType);
      setHTML('qbcdStatus2', _statusBadge(status));
      set('qbcdLatest', latest);

      // Position indicator
      var pos = document.getElementById('qbcdPos');
      if (pos) pos.textContent = _curIdx + 1 + ' of ' + _allSnaps.length;

      // Prev/Next button state
      var prevBtn = document.getElementById('qbcdPrevBtn');
      var nextBtn = document.getElementById('qbcdNextBtn');
      if (prevBtn) prevBtn.disabled = _curIdx <= 0;
      if (nextBtn) nextBtn.disabled = _curIdx >= _allSnaps.length - 1;
    }

    // ── Navigation ──────────────────────────────────────────────────────
    function _nav(dir) {
      var next = _curIdx + dir;
      if (next < 0 || next >= _allSnaps.length) return;
      _curIdx = next;
      _populate(_allSnaps[_curIdx]);
    }

    // ── Open modal ──────────────────────────────────────────────────────
    window._openQbCaseDetailModal = function(snap, allSnaps) {
      _allSnaps = Array.isArray(allSnaps) ? allSnaps : [snap];
      _curIdx = _allSnaps.findIndex(function(s) { return s && s.rowNum === snap.rowNum; });
    ...
    function _bind() {
      var closeBtn  = document.getElementById('qbcdCloseBtn');
      var closeBtn2 = document.getElementById('qbcdCloseBtn2');
      var prevBtn   = document.getElementById('qbcdPrevBtn');
      var nextBtn   = document.getElementById('qbcdNextBtn');
      var copyBtn   = document.getElementById('qbcdCopyBtn');

      if (closeBtn  && !closeBtn.__qbcdBound)  { closeBtn.__qbcdBound = true;  closeBtn.addEventListener('click',  function() { UI.closeModal('qbCaseDetailModal'); }); }
      if (closeBtn2 && !closeBtn2.__qbcdBound) { closeBtn2.__qbcdBound = true; closeBtn2.addEventListener('click', function() { UI.closeModal('qbCaseDetailModal'); }); }
      if (prevBtn   && !prevBtn.__qbcdBound)   { prevBtn.__qbcdBound = true;   prevBtn.addEventListener('click',   function() { _nav(-1); }); }
      if (nextBtn   && !nextBtn.__qbcdBound)   { nextBtn.__qbcdBound = true;   nextBtn.addEventListener('click',   function() { _nav(1); }); }
        ...
        copyBtn.addEventListener('click', function() {
          try {
            ...
            if (id && id !== '—') {
              ...
              }).catch(function() { copyBtn.textContent = 'Copy Case #'; });
            }
          } catch(_) {}
        });
      }
      ...
      if (!window.__qbcdKeyBound) {
        window.__qbcdKeyBound = true;
        document.addEventListener('keydown', function(e) {
          var modal = document.getElementById('qbCaseDetailModal');
          if (!modal || !modal.classList.contains('open')) return;
          if (e.key === 'Escape')      { UI.closeModal('qbCaseDetailModal'); }
