...
(function (global) {
  ...
  function checkForUpdates(force) {
    return Promise.all([
      ...
      // FIX (v3.9.28): Separate true hash-mismatch outdated from TTL-expired
      // bundles that just need a silent background re-fetch.
      // Previously, TTL-expiry pushed reason:'not_cached' into outdated[], which
      // triggered the banner on every login/refresh after 12-24 hours — even
      // though no data on the server had actually changed.
      var trulyOutdated = outdated.filter(function(o) { return o.reason !== 'not_cached'; });
      var silentRefresh = outdated.filter(function(o) { return o.reason === 'not_cached'; });

      // Auto-download expired bundles silently in the background
      if (silentRefresh.length > 0) {
        var _silentIds = silentRefresh.map(function(o) { return o.id; });
        setTimeout(function() {
          StudioCache.syncBundles(_silentIds, serverManifest)
            .catch(function(e) { console.warn('[StudioCache] silent refresh failed:', e.message); });
        }, 1500);
      }

      if (trulyOutdated.length > 0) {
        _emit('update', { outdated: trulyOutdated, serverManifest: serverManifest });
        return { status: 'update', outdated: trulyOutdated };
      }
